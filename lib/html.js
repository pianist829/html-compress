...
HTMLCompress.prototype.compress = function(){
    ...
    for (var i = 0, count = analyticContent.length; i < count; i++){
        ...
        if (tokenType === _.sign.FL_NEW_LINE) continue;
        switch (tokenType){
            ...
            case _.sign.HTML_JS_CONTENT :
                // 只有这样的script才会进行压缩：<script type="text/javscript" fcpcompress>
                var preTokenText = analyticContent[i - 1][0];
                if(/\sfcpcompress/i.test(preTokenText)) {
                    this._output[this._output.length] = this._tokenText;
                }else{
                    this._output[this._output.length] = this._tokenText;
                }
...
HTMLCompress.prototype._compressTextContent = function(){
    ...
    if (this._tokenText !== ''){
        ...
        if(this.level == 'strip:space'){
            this._tokenText = _.str_replace(["\r","\n","\t"], " ", this._tokenText);
        }else{
            this._tokenText = _.str_replace(["\r","\n","\t"], "",this._tokenText);
        }
        this._output[this._output.length] = _.trim(this._tokenText);
    }
};
